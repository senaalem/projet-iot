...
{
	assert(
			(lpp->cursor + CAYENNE_LPP_DIGITAL_INPUT_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE);
...
{
	assert(
			(lpp->cursor + CAYENNE_LPP_DIGITAL_OUTPUT_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE);
...
{
	assert(
			(lpp->cursor + CAYENNE_LPP_LUMINOSITY_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE);
...
{
	assert(
			(lpp->cursor + CAYENNE_LPP_PRESENCE_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE);
...
void cayenne_lpp_add_accelerometer(cayenne_lpp_t *lpp, uint8_t channel, float x,
		float y, float z)
{
	assert(
			(lpp->cursor + CAYENNE_LPP_ACCELEROMETER_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE);
...
{
	assert(
			(lpp->cursor + CAYENNE_LPP_BAROMETRIC_PRESSURE_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE);
...
{
	assert(
			(lpp->cursor + CAYENNE_LPP_GYROMETER_SIZE) < CAYENNE_LPP_MAX_BUFFER_SIZE);
